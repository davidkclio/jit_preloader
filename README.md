...
The JitPreloader can log each preload it performs to `Rails.logger.debug`. This is useful during development to verify that associations are being preloaded as expected. It is off by default.

```ruby
JitPreloader.debug = true
```

When enabled, each JIT preload will produce a log line like:

```
[JitPreloader] jit_preload: emails for 25 record(s)
```
