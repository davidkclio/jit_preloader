...
Apply the documentation changes you recommended. Edit the actual files (README.md, files in lib/, etc.) so the docs match the code. If you identified no changes, do nothing. Do NOT run any git commands (no commit, push, or branch); the script will create the branch and PR.
...
git push -u origin "$BRANCH" 2>/dev/null || git push --force-with-lease -u origin "$BRANCH"
